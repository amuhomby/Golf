...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
		...
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat space = 2.0;
            CGFloat imageWidth = 32;//icon.size.width;
            CGFloat imageHeight = 32;//icon.size.height;
            ...
            CGFloat imageYOffset = roundf((CGRectGetHeight(self.frame) - imageHeight-stringHeight- space - self.selectionIndicatorHeight) / 2.0f);
            yOffset = imageYOffset + imageHeight + space;

...
-(void)getData{
    ...
    _strBrand = [_strBrand stringByTrimmingCharactersInSet:
                  [NSCharacterSet whitespaceCharacterSet]];

    NSMutableDictionary * params = [[NSMutableDictionary alloc]init];
    [params setObject:user_id forKey:@"user_id"];
    [params setObject:_token forKey:@"_token"];
    [params setObject:_strBrand forKey:@"brand"];

    NSDictionary * result = [UtilComm getpostwithbrand:params];
    if(result != nil){
...
-(void)initView{
    [btnStyle setTitle:_strBrand forState:UIControlStateNormal];
    [btnStyle sizeToFit];
...
-(void)onClickAPhoto:(UIButton *)sender{
    ...
    NSString * otherid =[[arrAll objectAtIndex:index] objectForKey:@"user_id"];
    ...
    if([myfbid isEqualToString:otherid]){
        vc.fromView = @"";
    }else{

...
@property (nonatomic, strong) AddGalleryButton *addGallery;
...
- (void)addGalleryBtn:(UIButton *)btn;{
    [self composeInterface];
    barButtonItemSize = CGSizeMake([[UIScreen mainScreen] bounds].size.height * 0.045, [[UIScreen mainScreen] bounds].size.height * 0.045);
    btn.frame = (CGRect){self.frame.size.width,0, barButtonItemSize};// - barButtonItemSize.width - 10
    btn.center = CGPointMake(_topBarView.center.x * 1.40, _topBarView.center.y);
    //CGFloat y = _topBarView.frame.origin.y + _topBarView.center.y;
...
//    {
        ...
        addGalleryButtonSize = CGSizeMake(self.bounds.size.width * 0.07, self.bounds.size.width * 0.07);
...
//    }
    ...
    //Create shutter button
//    _addGallery = [AddGalleryButton new];
//    
//    if (_captureManager) {
//        
//        //Button Visual attribution
//        _addGallery.frame = (CGRect){0,0, shutterButtonSize};
//        _addGallery.center = CGPointMake(10, self.frame.size.height*0.88);
//        _addGallery.tag = AddGalleryButtonTag;
//        _addGallery.backgroundColor = [UIColor clearColor];
//        
//        //Button target
//        [_addGallery addTarget:self action:@selector(inputManager:) forControlEvents:UIControlEventTouchUpInside];
//        [self addSubview:_addGallery];
//    }
    ...
    if (_topBarView) {
        ...
        _topBarView.frame  = (CGRect){0,20, topBarSize};
        _topBarView.backgroundColor = [UIColor colorWithRed: 0.176 green: 0.478 blue: 0.529 alpha: 0.0];
        [self addSubview:_topBarView];
        
        //Add the camera toggle button
        _cameraToggle = [CameraToggleButton new];
        if (_cameraToggle) {
            _cameraToggle.frame = (CGRect){self.frame.size.width,0, barButtonItemSize};// - barButtonItemSize.width - 10
            _cameraToggle.center = CGPointMake(_topBarView.center.x * 1.0, _topBarView.center.y);
        ...
        if (_cameraFlash) {
            _cameraFlash.frame = (CGRect){self.frame.size.width,0, barButtonItemSize};// - barButtonItemSize.width * 2 - 20
            _cameraFlash.center = CGPointMake(_topBarView.center.x * 0.60, _topBarView.center.y);
        ...
        if (_captureManager) {
            ...
            barButtonItemSize = CGSizeMake([[UIScreen mainScreen] bounds].size.height * 0.0405, [[UIScreen mainScreen] bounds].size.height * 0.0405);
            _btnGallery.frame = (CGRect){self.frame.size.width,0, barButtonItemSize};// - barButtonItemSize.width - 10
            _btnGallery.center = CGPointMake(_topBarView.center.x * 1.40, _topBarView.center.y);
            //CGFloat y = _topBarView.frame.origin.y + _topBarView.center.y;
            
            [_topBarView addSubview:_btnGallery];
        }
...
-(void)inputManager:(id)sender {
    ...
    switch ([(UIButton *)sender tag]) {
        ...
        case AddGalleryButtonTag:  [self onTapDismissButton];  return;
    }
}

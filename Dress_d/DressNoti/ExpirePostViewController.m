...
@interface ExpirePostViewController ()<UITextFieldDelegate, UIScrollViewDelegate>
{
    BOOL _isObserving;
    float _oriStoryY;
    NSString * fbid;
...
@implementation ExpirePostViewController
...
-(void)initview{
    ...
    CGRect fbtnSetting = btnSetting.frame;
    fbtnSetting.origin.x = [UIScreen mainScreen].bounds.size.width - fbtnSetting.size.width - 14;
    btnSetting.frame = fbtnSetting;
...
-(IBAction)onClickSetting:(id)sender{
    ...
    NSString * photo = [_dicPostDetail objectForKey:@"photo"];
    photo = [NSString stringWithFormat:@"%@%@", PHOTO_URL, photo];
    vc.post_id = _post_id;
    vc.imgUrl =  photo;
    vc.subject = subject;
    [self.navigationController pushViewController:vc animated:YES];
